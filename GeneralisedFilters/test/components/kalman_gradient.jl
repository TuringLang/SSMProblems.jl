...
@testitem "Kalman gradient: ∂Q" begin
    ...
    ll_Q = GeneralisedFilters.GFTest.make_ll_func(s.model, s.ys, :Q)
    Q_vec = vec(Matrix(s.Q))
    ∂Q_fd = reshape(FiniteDifferences.grad(fdm, ll_Q, Q_vec)[1], s.D, s.D)
...
@testitem "Kalman gradient: ∂R" begin
    ...
    ll_R = GeneralisedFilters.GFTest.make_ll_func(s.model, s.ys, :R)
    R_vec = vec(Matrix(s.R))
    ∂R_fd = reshape(FiniteDifferences.grad(fdm, ll_R, R_vec)[1], s.D, s.D)
...
@testitem "Kalman gradient: ∂A" begin
    ...
    ll_A = GeneralisedFilters.GFTest.make_ll_func(s.model, s.ys, :A)
    A_vec = vec(Matrix(s.A))
    ∂A_fd = reshape(FiniteDifferences.grad(fdm, ll_A, A_vec)[1], s.D, s.D)
...
@testitem "Kalman gradient: ∂b" begin
    ...
    ll_b = GeneralisedFilters.GFTest.make_ll_func(s.model, s.ys, :b)
    ∂b_fd = FiniteDifferences.grad(fdm, ll_b, Vector(s.b))[1]
...
@testitem "Kalman gradient: ∂H" begin
    ...
    ll_H = GeneralisedFilters.GFTest.make_ll_func(s.model, s.ys, :H)
    H_vec = vec(Matrix(s.H))
    ∂H_fd = reshape(FiniteDifferences.grad(fdm, ll_H, H_vec)[1], s.D, s.D)
...
@testitem "Kalman gradient: ∂c" begin
    ...
    ll_c = GeneralisedFilters.GFTest.make_ll_func(s.model, s.ys, :c)
    ∂c_fd = FiniteDifferences.grad(fdm, ll_c, Vector(s.c))[1]
...
@testitem "Kalman gradient: ∂μ0" begin
    ...
    ll_μ0 = GeneralisedFilters.GFTest.make_ll_func(s.model, s.ys, :μ0)
    ∂μ0_fd = FiniteDifferences.grad(fdm, ll_μ0, Vector(s.μ0))[1]
...
@testitem "Kalman gradient: ∂Σ0" begin
    ...
    ll_Σ0 = GeneralisedFilters.GFTest.make_ll_func(s.model, s.ys, :Σ0)
    Σ0_vec = vec(Matrix(s.Σ0))
    ∂Σ0_fd = reshape(FiniteDifferences.grad(fdm, ll_Σ0, Σ0_vec)[1], s.D, s.D)
